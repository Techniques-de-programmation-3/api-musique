...
class modele_chanson {
    ...
    public function __construct($id, $titre, $artiste, $paroles, $album, $datePublication, $duree, $lectures) {
        ...
        $this->nombreLectures = intval($lectures);
    }
    ...
    public static function ObtenirListe($id_Liste) {
        ...
        $requete = $mysqli->prepare("SELECT chansons.id as id, titre, artiste, paroles, album, datePublication, duree, lectures from chansons inner join listes_chansons on chansons.id = listes_chansons.chanson_id WHERE listes_chansons.liste_id = ? ORDER BY ordre;");
